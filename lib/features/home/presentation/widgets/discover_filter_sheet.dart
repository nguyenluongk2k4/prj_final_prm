...
Future<DiscoverFilter?> showDiscoverFilterSheet(
  ...
  return showModalBottomSheet<DiscoverFilter>(
    ...
    builder: (_) => _DiscoverFilterSheet(currentFilter: currentFilter),
  );
}
...
class _DiscoverFilterSheet extends StatefulWidget {
  ...
  const _DiscoverFilterSheet({this.currentFilter});
...
class _DiscoverFilterSheetState extends State<_DiscoverFilterSheet> {
  int _interestedIndex = 2; // 0=Girls, 1=Boys, 2=Both
  double _distance = 40;
  ...
  void initState() {
    ...
    if (widget.currentFilter != null) {
      _distance = widget.currentFilter!.distanceKm ?? 40;
      _ageRange = RangeValues(
        widget.currentFilter!.ageMin?.toDouble() ?? 20,
        widget.currentFilter!.ageMax?.toDouble() ?? 28,
      );
      if (widget.currentFilter!.targetGender == 'female') {
        _interestedIndex = 0;
      } else if (widget.currentFilter!.targetGender == 'male') {
        _interestedIndex = 1;
      } else {
        _interestedIndex = 2;
      }
  ...
  void _apply() {
    String? targetGender;
    if (_interestedIndex == 0) {
      targetGender = 'female';
    } else if (_interestedIndex == 1) {
      targetGender = 'male';
    }
